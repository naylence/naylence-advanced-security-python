...
class TestChannelHandshakeFailureHandling:
    ...
    async def test_channel_handshake_failure_sends_delivery_nack(self):
        ...
        manager = ChannelEncryptionManager(secure_channel_manager=None, node_like=mock_node)
    ...
    async def test_signed_envelope_during_attachment_with_no_system_id(self):
        """Test that signed envelopes can be processed even when from_system_id is None
        (during attachment)."""
        ...
        signature = SignatureHeader(kid="test-signing-key-123", val="mock-signature-value")

        envelope_security = SecurityHeader(sig=signature)

        envelope = FameEnvelope(
            id="test-signed-envelope",
            to=FameAddress("destination@/test/destination"),
            reply_to=FameAddress("sender@/test/sender"),
            frame=data_frame,
            sec=envelope_security,
        )

        # Create delivery context WITHOUT from_system_id (simulating attachment phase)
        from naylence.fame.core.protocol.delivery_context import DeliveryOriginType, FameDeliveryContext

        context = FameDeliveryContext(
            origin_type=DeliveryOriginType.UPSTREAM,
            from_system_id=None,  # This is None during attachment
        )

        # Process the signed envelope - should not crash
        result = await envelope_security_handler.handle_signed_envelope(envelope, context)

        # Verify that it was processed successfully (key was available)
        assert result is True, "Signed envelope should be verified when key is available"

        # Verify that envelope verification was called
        envelope_verifier.verify_envelope.assert_called_once_with(envelope, check_payload=False)

        # Verify that has_key was called
        key_mgmt_handler.has_key.assert_called_once_with("test-signing-key-123")

        print("✅ Signed envelope during attachment test passed")

    @pytest.mark.asyncio
    async def test_signed_envelope_during_attachment_missing_key(self):
        """Test that signed envelopes are queued when key is missing and from_system_id is None."""
        print("🧪 Testing signed envelope queuing during attachment when key is missing")
        ...
        signature = SignatureHeader(kid="missing-signing-key-456", val="mock-signature-value")

        envelope_security = SecurityHeader(sig=signature)

        envelope = FameEnvelope(
            id="test-signed-envelope-missing-key",
            to=FameAddress("destination@/test/destination"),
            reply_to=FameAddress("sender@/test/sender"),
            frame=data_frame,
            sec=envelope_security,
        ...
        # Process the signed envelope - should queue it
        result = await envelope_security_handler.handle_signed_envelope(envelope, context)

        # Verify that it was queued (not processed)
        assert result is False, "Signed envelope should be queued when key is missing"

        # Verify that envelope was queued
        assert "missing-signing-key-456" in key_mgmt_handler._pending_envelopes
        assert len(key_mgmt_handler._pending_envelopes["missing-signing-key-456"]) == 1
        queued_envelope, queued_context = key_mgmt_handler._pending_envelopes["missing-signing-key-456"][0]
        assert queued_envelope == envelope
        assert queued_context == context

        # Verify that key request was made with placeholder system ID
        key_mgmt_handler._maybe_request_signing_key.assert_called_once_with(
            "missing-signing-key-456", DeliveryOriginType.UPSTREAM, "pending-attachment"
        )

        print("✅ Signed envelope queuing during attachment test passed")

    @pytest.mark.asyncio
    async def test_signed_envelope_during_attachment_with_missing_physical_path(self):
        """Test that signed envelopes are handled gracefully when both from_system_id
        and physical_path are unavailable."""
        print("🧪 Testing signed envelope handling when physical path is not available during attachment")

        # Create mock dependencies
        mock_node = Mock()
        mock_node.physical_path = Mock(side_effect=RuntimeError("Physical path not assigned yet"))
        mock_node.sid = "test-node-123"
        ...
        # Mock that key is NOT available (will trigger key request)
        key_mgmt_handler.has_key = AsyncMock(return_value=False)
        key_mgmt_handler._pending_envelopes = {}
        key_mgmt_handler._pending_key_requests = {}
        key_mgmt_handler._maybe_request_signing_key = AsyncMock()
        ...
        signature = SignatureHeader(kid="missing-key-during-attachment", val="mock-signature-value")

        envelope_security = SecurityHeader(sig=signature)

        envelope = FameEnvelope(
            id="test-signed-envelope-no-physical-path",
            to=FameAddress("destination@/test/destination"),
            reply_to=FameAddress("sender@/test/sender"),
            frame=data_frame,
            sec=envelope_security,
        ...
        # Process the signed envelope - should queue it and NOT crash when trying to request key
        result = await envelope_security_handler.handle_signed_envelope(envelope, context)

        # Verify that it was queued (not processed immediately)
        assert result is False, "Signed envelope should be queued when key is missing"

        # Verify that envelope was queued
        assert "missing-key-during-attachment" in key_mgmt_handler._pending_envelopes
        assert len(key_mgmt_handler._pending_envelopes["missing-key-during-attachment"]) == 1

        # Verify that key request was made with placeholder system ID
        key_mgmt_handler._maybe_request_signing_key.assert_called_once_with(
            "missing-key-during-attachment", DeliveryOriginType.UPSTREAM, "pending-attachment"
        )

        print("✅ Signed envelope handling during attachment with missing physical path test passed")
